<template>
  <q-infinite-scroll @load="onLoad" :offset="1000" :disable="disableScroll">
    <div v-if="displayedThread" :class="{ fixed: !mobile, toolbar: !mobile, 'mobile-toolbar': mobile }" style="z-index: 3"> 
      <q-toolbar style="margin-left:40px; cursor: pointer;" @click="displayThread(null)" class="q-py-lg">
        <svg width="22" height="15" viewBox="0 0 22 15" fill="none" xmlns="http://www.w3.org/2000/svg">
          ...
          <q-toolbar-title class="toolbar-header">{{ backButtonText }}</q-toolbar-title>
      </q-toolbar>
    </div>
    ...
    <div v-if="displayedThread" :style="{ 'z-index': 2, 'margin-top': mobile ? '' : '100px' }">
      <thread :thread="displayedThread" :show-comments="Boolean(displayedThread?.replies?.length)" />
    </div>

    <!-- Loading symbol at the bottom, indicates there is more items being loaded... -->
    <template v-if="!displayedThread && !disableScroll" v-slot:loading>
